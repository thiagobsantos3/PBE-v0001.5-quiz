-- Step 3: Harden function grants and add explicit auth checks

-- 1) Add auth guard to get_user_analytics_data
CREATE OR REPLACE FUNCTION public.get_user_analytics_data(
  p_user_id UUID,
  p_start_date TIMESTAMP WITH TIME ZONE DEFAULT NULL,
  p_end_date TIMESTAMP WITH TIME ZONE DEFAULT NULL
)
RETURNS TABLE (
  total_quizzes_completed BIGINT,
  total_questions_answered BIGINT,
  average_score NUMERIC,
  total_time_spent_minutes BIGINT,
  study_streak INTEGER,
  total_points_earned BIGINT,
  total_possible_points BIGINT
)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
DECLARE
  v_start_date TIMESTAMP WITH TIME ZONE;
  v_end_date TIMESTAMP WITH TIME ZONE;
  v_caller uuid := auth.uid();
BEGIN
  -- Only the user themself or an admin can query analytics for a user
  IF NOT (p_user_id = v_caller OR COALESCE(public.is_admin(), false)) THEN
    RAISE EXCEPTION 'Not authorized to access analytics for this user';
  END IF;

  v_start_date := COALESCE(p_start_date, NOW() - INTERVAL '30 days');
  v_end_date := COALESCE(p_end_date, NOW());
  
  RETURN QUERY
  WITH filtered AS (
    SELECT *
    FROM public.quiz_sessions
    WHERE user_id = p_user_id
      AND status = 'completed'
      AND completed_at BETWEEN v_start_date AND v_end_date
  )
  SELECT
    COUNT(*)::bigint,
    COALESCE(SUM(
      CASE WHEN results IS NOT NULL AND jsonb_typeof(results) = 'array'
           THEN jsonb_array_length(results)
           ELSE 0 END
    ), 0)::bigint,
    CASE WHEN COALESCE(SUM(max_points), 0) > 0
         THEN ROUND(COALESCE(SUM(total_points), 0) * 100.0 / COALESCE(SUM(max_points), 0), 2)
         ELSE 0 END,
    COALESCE(SUM(
      CASE WHEN total_actual_time_spent_seconds IS NOT NULL AND total_actual_time_spent_seconds > 0
           THEN (total_actual_time_spent_seconds / 60)
           WHEN estimated_minutes IS NOT NULL AND estimated_minutes > 0
           THEN estimated_minutes
           ELSE GREATEST(1,
             CASE WHEN questions IS NOT NULL AND jsonb_typeof(questions) = 'array'
                  THEN jsonb_array_length(questions) * 2
                  ELSE 2 END)
      END
    ), 0)::bigint,
    (
      WITH daily_activity AS (
        SELECT DISTINCT DATE(completed_at) AS study_date
        FROM filtered
      ),
      streak_calc AS (
        SELECT
          study_date,
          ROW_NUMBER() OVER (ORDER BY study_date DESC) AS rn,
          study_date - (ROW_NUMBER() OVER (ORDER BY study_date DESC) || ' days')::INTERVAL AS grp
        FROM daily_activity
      )
      SELECT COALESCE(COUNT(*), 0)
      FROM streak_calc
      WHERE grp = (SELECT grp FROM streak_calc ORDER BY study_date DESC LIMIT 1)
    )::integer,
    COALESCE(SUM(total_points + COALESCE(bonus_xp, 0)), 0)::bigint,
    COALESCE(SUM(max_points), 0)::bigint
  FROM filtered;
END;
$$;

-- 2) Revoke EXECUTE from PUBLIC on high-risk functions and grant to authenticated
-- Note: repeat grants are safe; revokes fail if signature mismatches, so keep signatures in sync

-- Leaderboard
REVOKE EXECUTE ON FUNCTION public.get_team_leaderboard_data(uuid, timestamptz, timestamptz) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_team_leaderboard_data(uuid, timestamptz, timestamptz) TO authenticated;

-- Delete RPC
REVOKE EXECUTE ON FUNCTION public.delete_quiz_and_adjust_gamification(uuid, uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.delete_quiz_and_adjust_gamification(uuid, uuid) TO authenticated;

-- User analytics
REVOKE EXECUTE ON FUNCTION public.get_user_analytics_data(UUID, TIMESTAMPTZ, TIMESTAMPTZ) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_user_analytics_data(UUID, TIMESTAMPTZ, TIMESTAMPTZ) TO authenticated;

-- Helper auth functions
REVOKE EXECUTE ON FUNCTION public.can_access_team(uuid) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.is_team_owner(uuid) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.can_manage_team(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.can_access_team(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.is_team_owner(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.can_manage_team(uuid) TO authenticated;

-- Admin helper
REVOKE EXECUTE ON FUNCTION public.is_admin() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.is_admin() TO authenticated;

-- Minimal profile name getter
REVOKE EXECUTE ON FUNCTION public.get_user_name_by_id(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_user_name_by_id(uuid) TO authenticated;