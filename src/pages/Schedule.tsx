...
export function Schedule() {
  ...
  useEffect(() => {
    if (teamMembers.length > 0 && !selectedViewingMemberId && user) {
      // Log the default selection logic for admins/owners
      developerLog('🔍 Schedule: Initializing selectedViewingMemberId:', {
        userId: user.id,
        userTeamRole: user.teamRole,
        // Assuming 'undefined' means all team, adjust if your logic is different
        defaultingTo: user.teamRole === 'member' ? user.id : 'undefined (all team)', 
        currentSelectedViewingMemberId: selectedViewingMemberId // Should be empty initially
      });
  ...
  // Log memberAssignments before passing to useCalendar
  developerLog('🔍 Schedule: memberAssignments being passed to useCalendar:', {
    selectedMemberId: selectedViewingMemberId,
    count: memberAssignments.length,
    assignments: memberAssignments.map(a => ({
      id: a.id,
      date: a.date.toISOString(), // Log as ISO string for precision
      userId: a.user_id,
  ...
  const handleStartQuiz = (assignment: StudyAssignment) => {
    // Navigate to QuizRunner for study assignments
