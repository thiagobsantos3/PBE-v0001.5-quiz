...
export function ResetPassword() {
  ...
  const [accessToken, setAccessToken] = useState<string | null>(null); // JWT from #access_token or ?access_token
  const [recoveryToken, setRecoveryToken] = useState<string | null>(null); // short token from ?token
  ...
  // Parse URL hash or query for Supabase auth parameters
  useEffect(() => {
    ...
    const jwtToken = hashParams.access_token || queryParams.access_token || null;
    const shortRecoveryToken = queryParams.token || hashParams.token || null;
    const type = hashParams.type || queryParams.type;
    const rtoken = hashParams.refresh_token || queryParams.refresh_token;
    ...
    if (shortRecoveryToken) setRecoveryToken(shortRecoveryToken);
    if (rtoken) setRefreshToken(rtoken);
    setResetType(type);
    setTokenValidated(!!jwtToken || !!shortRecoveryToken);

    // If both tokens are present, proactively set the session so updateUser can succeed tokenlessly
    (async () => {
      try {
        if (jwtToken && rtoken) {
          const { error } = await supabase.auth.setSession({ access_token: jwtToken, refresh_token: rtoken });
          if (error) {
            console.error('Supabase setSession error on reset page:', error);
            setError('Invalid or expired reset token');
          }
        }
      } catch (e) {
        console.error('Supabase setSession exception on reset page:', e);
      }
    })();
  ...
  const onSubmit = async (data: ResetPasswordForm) => {
    // If we only have the short recovery token, verify it to establish a session
    ...
    try {
      ...
      // Try to use both access and refresh token when present (Supabase may include refresh_token)
      if (!accessToken && recoveryToken) {
        // Establish a session using the short token
        const { error: verifyError } = await supabase.auth.verifyOtp({ type: 'recovery', token: recoveryToken });
        if (verifyError) {
          throw verifyError;
        }
      }
      await resetPasswordWithToken(accessToken, data.password, refreshToken || undefined);
      setSuccess(true);
