      ...
      if (editingPlan) {
        ...
        const { error: settingsUpdateError } = await supabase
          ...
          .update({
            ...
            allow_mock_test_creation: settingsFormData.allow_mock_test_creation,
            allow_test_assignments: settingsFormData.allow_test_assignments,
            updated_at: new Date().toISOString()
          })
      ...
      render: (plan) => {
        ...
        return (
          <div className="text-sm text-gray-900 flex flex-col space-y-1">
            ...
            <span className="flex items-center">
              {settings?.allow_mock_test_creation ? <CheckCircle className="h-4 w-4 text-green-500 mr-1" /> : <XCircle className="h-4 w-4 text-red-500 mr-1" />} Mock Test Creation
            </span>
            <span className="flex items-center">
              {settings?.allow_test_assignments ? <CheckCircle className="h-4 w-4 text-green-500 mr-1" /> : <XCircle className="h-4 w-4 text-red-500 mr-1" />} Test Assignments
            </span>
          </div>
        );
      },
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="md:col-span-2">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                    ...
                    />
                    <FormField
                      label="Allow Mock Test Creation"
                      id="allowMockTestCreation"
                      type="select"
                      value={settingsFormData.allow_mock_test_creation?.toString() || 'false'}
                      onChange={(val) => setSettingsFormData({ ...settingsFormData, allow_mock_test_creation: val === 'true' })}
                      options={[
                        { value: 'true', label: 'Yes' },
                        { value: 'false', label: 'No' },
                      ]}
                    />
                    <FormField
                      label="Allow Test Assignments"
                      id="allowTestAssignments"
                      type="select"
                      value={settingsFormData.allow_test_assignments?.toString() || 'false'}
                      onChange={(val) => setSettingsFormData({ ...settingsFormData, allow_test_assignments: val === 'true' })}
                      options={[
                        { value: 'true', label: 'Yes' },
                        { value: 'false', label: 'No' },
                      ]}
