...
import { getAccessibleQuestions, getChaptersForBook, getAvailableBooksFromQuestions, selectAndDistributeQuestions } from '../utils/quizUtils';
...
export function CreateOwnQuiz() {
  ...
  useEffect(() => {
    ...
    if (user.planSettings && !user.planSettings.allow_create_own_quiz) { // This check is correct
  ...
  const getFilteredQuestions = React.useCallback((): Question[] => {
    ...
    // Use the centralized question selection utility
    return selectAndDistributeQuestions(
      questions,
      selectedBooks,
  ...
  const getAvailableQuestionsCount = React.useCallback((): number => {
    // Get all eligible questions before any selection limits
    let allEligible: Question[] = [];
    ...
    selectedBooks.forEach(book => {
      ...
      if (bookChapters.length > 0) {
        ...
        allEligible = [...allEligible, ...bookQuestions];
      }
    });

    return getAccessibleQuestions(allEligible, user?.subscription?.plan || 'free').length;
  ...
  const handleSessionDeleted = () => {
    ...
    navigate('/quiz'); // Redirect to the main Quiz Center
  };
