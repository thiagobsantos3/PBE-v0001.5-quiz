...
export function StudyScheduleQuiz() {
  ...
  const handleResumeQuiz = React.useCallback(() => {
    ...
    if (existingSession) {
      // Navigate to QuizRunner for study assignments
      navigate(`/quiz/runner/${existingSession.id}`);
    }
  ...
  const handleDeleteSession = React.useCallback(() => {
    ...
    // Navigate back to schedule since we're no longer using local quiz session state
    navigate('/schedule');
  }, []);
  ...
  useEffect(() => {
    ...
    if (user.planSettings && !user.planSettings.allow_study_schedule_quiz) { // This check is correct
  ...
  if (quizSessionId) {
    // Study assignments now use TestRunner, so redirect if we somehow get here
