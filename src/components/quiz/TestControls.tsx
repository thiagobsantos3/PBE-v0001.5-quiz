...
import { Send, Eye, EyeOff, Check, X } from 'lucide-react';
...
interface TestControlsProps {
  ...
  onCorrect: () => void;
  onIncorrect: () => void;
  isReviewPhase?: boolean; // Whether we're in the final review phase
}
...
export function TestControls({
  ...
  // During the test (not review phase), show submit button
  if (!isReviewPhase) {
    if (isFullScreen) {
      return (
        <div className="fixed bottom-4 sm:bottom-6 left-0 right-0 z-20 px-4 sm:px-6">
          ...
          <div className="sm:hidden flex justify-center">
            <button
              ...
              className={`${themeClasses.button} px-6 py-3 rounded-lg transition-colors duration-200 flex items-center space-x-2 text-lg w-full max-w-xs justify-center disabled:opacity-50 disabled:cursor-not-allowed`}
            >
          ...
          <div className="hidden sm:flex justify-center">
            <button
              onClick={onSubmitAnswer}
              disabled={!typedAnswer.trim()}
              className={`${themeClasses.button} px-8 py-4 rounded-lg transition-colors duration-200 flex items-center space-x-2 text-[2.5vw] disabled:opacity-50 disabled:cursor-not-allowed`}
            >
              <Send className="h-[2vw] w-[2vw]" />
              <span>Submit Answer</span>
            </button>
          </div>
        </div>
    ...
    // Regular submit button for non-fullscreen mode
    return (
      <div className="flex justify-center mt-6">
        <button
          onClick={onSubmitAnswer}
          disabled={!typedAnswer.trim()}
          className="bg-indigo-600 text-white px-8 py-3 rounded-lg text-lg font-medium hover:bg-indigo-700 transition-colors duration-200 flex items-center space-x-2 disabled:opacity-50 disabled:cursor-not-allowed"
        >
          <Send className="h-5 w-5" />
          <span>Submit Answer</span>
        </button>
  ...
  // During review phase, use the same controls as regular quizzes
  if (isFullScreen) {
    return (
      <div className="fixed bottom-4 sm:bottom-6 left-0 right-0 z-20 px-4 sm:px-6">
        {showAnswer ? (
  ...
  // Regular buttons for non-fullscreen mode during review
  return (
    <div className="flex justify-center mt-6">
      {!showAnswer ? (
        <button
