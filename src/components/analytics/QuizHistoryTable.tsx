...
import { BookOpen, Calendar, Target, Trophy, Clock, AlertTriangle, CheckCircle, XCircle, Trash2, ChevronLeft, ChevronRight } from 'lucide-react';
...
export function QuizHistoryTable({ 
  ...
  const handleDeleteQuiz = async (sessionId: string, quizTitle: string) => {
    ...
    const confirmed = window.confirm(
      ...
      '• Delete the associated study assignment (if any)\n' +
        ...
        <div className="text-center">
          {entry.approval_status ? (
            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
              ...
              <span className="capitalize">{entry.approval_status}</span>
            </span>
          ) : (
        ...
        <div className="p-4 bg-gray-50 border-t border-gray-200">
          <div className="flex items-center justify-between">
            <div className="text-sm text-gray-600">
              Showing {startItem}-{endItem} of {totalCount} quizzes
            </div>
          ...
          <div className="text-center mt-2">
            <p className="text-sm text-gray-600">
              Click on a row to view detailed results.
            </p>
          </div>
        </div>
      )}
