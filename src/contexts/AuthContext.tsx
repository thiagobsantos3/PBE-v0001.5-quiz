...
export function AuthProvider({ children }: { children: ReactNode }) { 
  ...
  useEffect(() => {
    ...
    const initializeAuth = async () => {
      ...
      // If a password recovery hash is present but the path is not /reset-password,
      // rewrite the URL to the correct route while preserving the hash (no reload)
      if (typeof window !== 'undefined') {
        const currentHash = window.location.hash || '';
        const currentPath = window.location.pathname;
        if (currentHash.includes('type=recovery') && currentPath !== '/reset-password') {
          developerLog('🔁 Detected recovery hash on wrong path; rewriting to /reset-password');
          try {
            window.history.replaceState(null, '', `/reset-password${currentHash}`);
          } catch (e) {
            // Fallback to a hard navigation if replaceState fails for any reason
            window.location.replace(`/reset-password${currentHash}`);
            return; // bail out; page will navigate
          }
        }
      }
      ...
      if (sessionError && sessionError.message?.includes('Refresh Token Not Found')) {
        developerLog('🔄 Invalid refresh token detected, clearing session...');
      ...
      if (session?.user) {
        // If we're currently on the recovery route, defer profile loading entirely
        const isRecoveryRoute = typeof window !== 'undefined'
          && window.location.pathname === '/reset-password';

        if (isRecoveryRoute) {
          developerLog('🔐 Initial session present during recovery; deferring profile load');
          setHasSession(false);
          setProfileLoading(false);
          setLoading(false);
        } else {
    ...
    const {
      ...
      if (event === 'SIGNED_OUT' || event === 'USER_DELETED') {
        ...
        // During password recovery, let the /reset-password route handle the flow without forcing profile load
        developerLog('🔐 Password recovery event detected; awaiting reset on /reset-password');
        setHasSession(false);
        setProfileLoading(false);
        setLoading(false);
      } else if (session?.user) { // For SIGNED_IN, USER_UPDATED
        // If we're on the reset password route handling a recovery flow, do not load profile yet
        const isRecoveryRoute = typeof window !== 'undefined'
          && window.location.pathname === '/reset-password';
        if (isRecoveryRoute) {
          developerLog('🔐 Recovery session on /reset-password; deferring profile load');
          setHasSession(false);
          setProfileLoading(false);
          setLoading(false);
          return;
  ...
  const fetchPlanSettingsForPlan = async (planId: string): Promise<PlanSettings | null> => {
    try {
      ...
      const { data: settings, error: settingsError } = await supabase
        .from('plan_settings')
        .select('plan_id, max_team_members, max_questions_custom_quiz, question_tier_access, allow_quick_start_quiz, allow_create_own_quiz, allow_study_schedule_quiz, allow_analytics_access, allow_mock_test_creation, allow_test_assignments')
        .eq('plan_id', planId)
        .maybeSingle();
  ...
  const resetPasswordWithToken = async (accessToken: string | null | undefined, newPassword: string, refreshToken?: string) => {
    try {
      logLoadingState('resetPasswordWithToken start', true, 'Updating password');
      developerLog('🔄 Resetting password (token-aware)...');
      ...
      if (accessToken) {
        if (refreshToken) {
          // Only set session if both tokens are present
          const { data: sessionResponse, error: setSessionError } = await supabase.auth.setSession({
            access_token: accessToken,
            refresh_token: refreshToken
          });

          if (setSessionError || !sessionResponse.user) {
            developerLog('❌ Error setting session with reset token:', setSessionError);
            logLoadingState('resetPasswordWithToken session error', false, setSessionError?.message || 'Invalid reset token');
          ...
          // No refresh token; assume session is already established (via verifyOtp or SDK) and update directly
          developerLog('🔐 Access token present but no refresh token; attempting direct password update');
          const result = await supabase.auth.updateUser({ password: newPassword });
          updateError = result.error;
          updateData = result.data;
        }
      } else {
        // Tokenless recovery flow: Supabase flags the session as PASSWORD_RECOVERY; just update password
        developerLog('🔐 No access token present; attempting recovery update directly');
        const result = await supabase.auth.updateUser({ password: newPassword });
        updateError = result.error;
        updateData = result.data;
      }
      ...
      if (updateData?.user) {
        ...
        if (currentSession.session?.user) {
          await loadUserProfile(currentSession.session.user.id, currentSession.session.user.email);
        }
      }
